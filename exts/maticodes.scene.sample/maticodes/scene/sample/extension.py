    ...
    def on_build(self):
        ...
        with sc.Transform(transform=sc.Matrix44.get_translation_matrix(*self.model.position.value)):
            with sc.Transform(look_at=sc.Transform.LookAt.CAMERA):
                ...
                sc.Line([0, -self._half_line_length, 0], [0, self._half_line_length, 0], color=cl.yellow, thickness=self._thickness)
                sc.Line([-self._half_line_length, 0, 0], [self._half_line_length, 0, 0], color=cl.yellow, thickness=self._thickness)
...
class CameraModel(sc.AbstractManipulatorModel):
    def __init__(self):
        ...
        self._stage_event_sub = omni.usd.get_context().get_stage_event_stream().create_subscription_to_pop(on_usd_context_event, name="CameraModel stage event")
        stage = omni.usd.get_context().get_stage()
        if stage:
...
class SampleWindow(ui.Window):
    ...
    def __build_window(self):
        scene_view = sc.SceneView(model=CameraModel(), aspect_ratio_policy=sc.AspectRatioPolicy.PRESERVE_ASPECT_HORIZONTAL, scene_aspect_ratio=1280 / 720)
